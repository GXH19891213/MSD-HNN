...
class HypergraphConv(nn.Module):
    def __init__(self, input_dim, output_dim, bias=True, drop_rate=0.5):
        super().__init__()
        self.act = nn.ReLU(inplace=True)  # 激活函数ReLU，inplace表示直接修改输入变量而不返回新的变量，节省内存
        self.drop = nn.Dropout(drop_rate)  # Dropout层，用于正则化
        self.theta = nn.Linear(input_dim, output_dim, bias=bias)  # 特征变换的线性层

    def forward(self, X, hypergraph):
        X = self.theta(X)  # 线性变换
        Y = hypergraph.v2e(X, aggr="mean")  # 从节点聚合到超边
        X_ = hypergraph.e2v(Y, aggr="mean")  # 从超边聚合回节点
        X_ = self.drop(self.act(X_))  # 激活函数+Dropout
        return X_  # 返回更新后的节点特征
