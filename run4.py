...
parser.add_argument('-data_name', default='gos')
...
parser.add_argument('-user_size', type=int, default=65064)
    ...
    for i, (batch, batch1) in enumerate(zip(training_data, training1_data)):
        try:
            ...
            for i, hg in enumerate(hypergraph_list):
                print(f"run4.py: 超图 {i} - 节点数: {hg.num_v}, 边数: {hg.num_e}")
...
def train_model(JointModel, data_name):
    ...
    for epoch_i in range(opt.epoch):
        ...
        if epoch_i > 8:
            # 计算验证集和测试集的得分
            scores = test_epoch(model, valid_data, valid1_data)
            print('  - (Validation)')
            for metric, value in scores.items():
                print(f'{metric}: {value * 100:.5f}%')

            print('  - (Test)')
            scores = test_epoch(model, test_data, test1_data)
            for metric, value in scores.items():
                print(f'{metric}: {value * 100:.5f}%')
            ...
            updated = False
            for metric, value in scores.items():
                if value * 100 < 100:  # **如果当前值等于100%，则不更新**
                    if metric not in best_scores or value > best_scores[metric]:  
                        best_scores[metric] = value
                        updated = True  # 只有有更新时才保存模型
