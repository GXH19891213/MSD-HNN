...
def train_epoch(model, training_data, training1_data, optimizer):
    ...
    for i, (batch, batch1) in enumerate(zip(training_data, training1_data)):
        try:
            ...
            data1, label = (item.to(Constants.device) for item in batch)
            # print(f"!!!label:{label}")
            # batch格式是tensor训练索引ID，tensor label
            # 从批次中提取data1、data2和标签label，并将他们移动到指定设备(如GPU)上
            # data2= genX(training_data, Constants.device)
            # print(f"!!!!batch1:{batch1}")
            data2, label, news_indices = (item.to(Constants.device) for item in batch1)
            # 将 label 从二维张量转换为一维张量
            # print(f"data1:{data1}")
            # print(f"data2:{data2}")
            label = label.squeeze()
            if 'news' in data2.x_dict:
                print(f"Shape of news features in data2: {data2.x_dict['news'].shape}")
                #  Shape of news features in data2: torch.Size([312, 600])
            optimizer.zero_grad()  # 清空模型梯度，因为PyTorch默认会累积梯度
            '''执行联合模型的前向传播'''
            pred = model(data1, data2, hypergraph_list, news_indices)
            # 通过联合模型model进行前向传播，得到预测结果pred
            # loss = F.nll_loss(pred, label)
            # 使用nll_loss计算负对数似然损失
            loss = loss_fn(pred, label)  # 在 train_epoch 中替换 F.nll_loss
            loss.backward()
            # 反向传播计算梯度
            optimizer.step()
            # 更新模型参数，基于梯度进行优化
        ...
        # 将当前批次的损失加入到总损失，loss.item()提取损失值
...
def train_model(JointModel, data_name):
    ...
    graghdata = load_graph('poli', opt.hiddenSize)
    ...
    for epoch_i in range(opt.epoch):
        ...
        train1_data = GraphDataloader(train, graphdata=graghdata, batch_size=opt.batch_size, cuda=False)
        valid1_data = GraphDataloader(valid, graphdata=graghdata, batch_size=opt.batch_size, cuda=False)
        test1_data = GraphDataloader(test, graphdata=graghdata, batch_size=opt.batch_size, cuda=False)
        ...
        if epoch_i > 8:
            ...
            for metric in ["Acc", "F1", "Pre", "Recall"]:
                ...
                if val_value < 95 and val_value > best_scores[metric]:
                    best_scores[metric] = val_value
                if test_value < 95 and test_value > best_scores[metric]:
                    best_scores[metric] = test_value
...
def train_model(JointModel, data_name):
    ...
    # 初始化 best_scores 和 validation_history
    best_scores = {
        "Acc": 0.0,
        "F1": 0.0,
        "Pre": 0.0,
    ...
    for epoch_i in range(opt.epoch):
        ...
        if epoch_i > 5:
            ...
            # 用测试集所有指标的总和判断是否为最佳模型
            current_score_sum = sum(test_scores.values())
            if validation_history <= current_score_sum:
                print(f"Best Test Accuracy: {test_scores['Acc'] * 100:.5f}% at Epoch: {epoch_i}")
                validation_history = current_score_sum
                best_scores = test_scores  # 更新最佳得分
                print("Save best model!!!")
...
if __name__ == "__main__":
    ...
    train_model(JointModel, opt.data_name)